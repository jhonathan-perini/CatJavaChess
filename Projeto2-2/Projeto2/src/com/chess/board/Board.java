...
public class Board {
    ...
    private final List<TileBoard> gameBoard;
    ...
    public String toString(){
     ...
     for(int i=0; i<Utils.NUM_TILES; i++){
         ...
         if((i+1) % Utils.NUM_TILES_PER_ROW ==0){
             builder.append("\n");
    ...
    private static String prettyPrint(final TileBoard tile) {
        if(tile.isTileNotEmpty()){
    ...
    private Collection<ChessPiece> calculateActivePieces(final List<TileBoard> gameBoard, final Alliance alliance) {
        final List<ChessPiece> activePieces = new ArrayList<>();

        for(final TileBoard tile: gameBoard) {
            if(tile.isTileNotEmpty()){
    ...
    public TileBoard getTile(final int tileCoordinate){
    return gameBoard.get(tileCoordinate);
    }

    private static List<TileBoard> createGameBoard(final Builder builder) {
        final TileBoard[] tiles = new TileBoard[Utils.NUM_TILES];
        for(int i=0; i<Utils.NUM_TILES; i++){
            tiles[i] = TileBoard.createTile(i, builder.boardConfig.get(i));
        }
        return ImmutableList.copyOf(tiles); //VERIFICAR
    }
